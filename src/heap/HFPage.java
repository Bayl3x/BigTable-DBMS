...
import static global.GlobalConst.MAP_SIZE;
...
public class HFPage extends Page 
  ...
  // Will we need to change any of these values?
...
//            Tuple tuple = new Tuple(record, 0, recLen);
  ...
  public MAP returnMap ( MID mid )
      ...
      //read the page in to a Page object
      try {
              SystemDefs.JavabaseBM.pinPage(mid.pageNo, apage, false);
          ...
          buffer  = apage.getpage();

      //go to where the map is and make the MAP object
      byte[] tmp = new byte[MAP_SIZE];
      tmp = Arrays.copyOfRange(buffer, MAP_SIZE * mid.slotNo, (mid.slotNo + 1) * MAP_SIZE);
      MAP map = new MAP(tmp);

      //unpin the page
      try {
          SystemDefs.JavabaseBM.unpinPage(mid.pageNo, false);
      } catch (ReplacerException e) {
          e.printStackTrace();
      } catch (PageUnpinnedException e) {
          e.printStackTrace();
